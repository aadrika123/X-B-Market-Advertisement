...
class MarShopDemand extends Model
{
    ...
    {
        return self::select('id', 'financial_year', DB::raw('ROUND(amount, 2) as amount'), 'payment_status', DB::raw("TO_CHAR(payment_date, 'DD-MM-YYYY') as payment_date"), 'tran_id')->where('shop_id', $shopId)->where('status', 1)->orderBy('financial_year', 'ASC')->get();
    }
